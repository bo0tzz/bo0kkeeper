...
export class SettingsService implements OnModuleInit {
  ...
  private async ensureInitialized(): Promise<AppSettings> {
    ...
    const seeded = await this.repository.create({
      ...
      issuerAddressLine1: env.ISSUER_ADDRESS_LINE1 ?? 'Example Street 1',
      issuerPostalCode: env.ISSUER_POSTAL_CODE ?? '1234 AB',
      issuerCity: env.ISSUER_CITY ?? 'Exampletown',
      issuerCountry: env.ISSUER_COUNTRY ?? 'The Netherlands',
