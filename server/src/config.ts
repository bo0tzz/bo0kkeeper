...
const ConfigSchema = z
  ...
  // off (operator error, env-var typo, drift from staging) anyone can forge
  // events that drive downstream side effects (drafts, sheet writes). Refuse to
  // boot rather than silently accept unauthenticated webhooks.
  .refine((c) => c.NODE_ENV !== 'production' || (c.WISE_WEBHOOK_VERIFY && !!c.WISE_WEBHOOK_PUBLIC_KEY), {
    error:
      'In NODE_ENV=production, WISE_WEBHOOK_VERIFY must be true and WISE_WEBHOOK_PUBLIC_KEY must be set — unauthenticated webhooks are refused',
    path: ['WISE_WEBHOOK_VERIFY'],
  })
  .refine((c) => c.NODE_ENV !== 'production' || !!c.PAPERLESS_WEBHOOK_TOKEN, {
    error: 'In NODE_ENV=production, PAPERLESS_WEBHOOK_TOKEN must be set — unauthenticated webhooks are refused',
    path: ['PAPERLESS_WEBHOOK_TOKEN'],
  });
